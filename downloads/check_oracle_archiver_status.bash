...
print_help(){
	...
	echo "In this case, the sql qill check for Archiver status. If no rows are returned, an"
	echo "alert should be triggered."
	echo ""
	echo "    Usage:"
...
if [[ -n $DB && -n $USER && -n $PASSWD ]]; then
	...
	if [[ $RESULT =~ "ERROR" ]]; then 
		...
		echo "WARNING - $RESULT"
		exit $WARNING		
	elif [[ $RESULT =~ "no rows selected" ]]; then
		echo "CRTICAL - Archiver not started";
		exit $CRITICAL
	elif [[ -z $RESULT ]]; then
		echo "WARNING - Empty string returned"
		exit $WARNING
	elif [[ $RESULT =~ "1 ---------- 1" ]]; then
		echo "OK - Archiver started"
		exit $OK
	else
